...
| `test_compaction` | End-to-end (fake provider + real engine): trigger fires → checkpoint committed → post-compaction request excludes pre-checkpoint turns and carries the checkpoint block. Store round-trip: pending/complete/failed visibility, chain via `previous_checkpoint_id`. Messages survive compaction (row count unchanged). Assembly slicing + alternation. Chained split/prompt merge. `get_context_window` override, hysteresis gating |
